...
vi.mock('@/infrastructure/ai/services/ai-log.admin.service', () => ({ createAiLog: vi.fn(), updateAiLog: vi.fn() }));
vi.mock('@/infrastructure/ai/services/ai-lock.admin.service', () => ({ releaseRequestLock: vi.fn() }));
...
describe('streamSubtasksForTask', () => {
  ...
  it('passes the supplied signal to the provider', async () => {
    const provider = createProvider([{ type: 'done', metadata: { model: 'test', response: '[]', finishReason: 'stop', usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2, reasoningTokens: 0, cacheHitTokens: 0, cacheMissTokens: 0, durationMs: null } } }]);
    const providerStream = vi.spyOn(provider, 'stream');

    const { stream } = await streamSubtasksForTask({ task, userId: 'user-1', provider, signal });
  ...
  it('streams subtasks and records successful completion metadata', async () => {
    const metadata = { model: 'deepseek-v4-flash', response: '[{"title":"Book hotel"}]', finishReason: 'tool_calls', usage: { inputTokens: 10, outputTokens: 20, totalTokens: 30, reasoningTokens: 0, cacheHitTokens: 0, cacheMissTokens: 0, durationMs: null } };
    const provider = createProvider([{ type: 'subtask', subtask: { title: 'Book hotel' } }, { type: 'done', metadata }]);
    const { stream, aiLogId } = await streamSubtasksForTask({ task, userId: 'user-1', provider, signal });

    expect(aiLogId).toBe('log-1');
    expect(await collectResponse(stream)).toBe('{"type":"subtask","subtask":{"title":"Book hotel"}}\n{"type":"done"}\n');
    expect(mockedGetSuccessAiLogs).toHaveBeenCalledWith(metadata, metadata.response);
    expect(mockedUpdateAiLog).toHaveBeenCalledWith('log-1', { status: 'success' });
  ...
  it('does not emit an error or completion after cancellation', async () => {
    ...
    const { stream } = await streamSubtasksForTask({ task, userId: 'user-1', provider, signal: controller.signal });
  ...
  it('records a failed generation and releases the lock when the provider fails', async () => {
    ...
    const provider: AIProvider = { generate: vi.fn(), stream: async function* () { throw error; } };
    const { stream } = await streamSubtasksForTask({ task, userId: 'user-1', provider, signal });
    const reader = stream.getReader();
    const decoder = new TextDecoder();
    ...
    expect(JSON.parse(decoder.decode(first.value))).toEqual({ type: 'error', error: { success: false, status: 502, code: 'AI_GENERATION_FAILED', message: 'provider failed' } });
    expect((await reader.read()).done).toBe(true);
    expect(mockedGetFailedAiLogs).toHaveBeenCalledWith(expect.objectContaining({ code: 'AI_GENERATION_FAILED', message: 'provider failed' }));
    expect(mockedUpdateAiLog).toHaveBeenCalledWith('log-1', { status: 'failed' });
    expect(mockedReleaseRequestLock).toHaveBeenCalledWith('user-1');
  });
  ...
  it('releases the lock when logging fails after a successful generation', async () => {
    ...
    const provider = createProvider([{ type: 'done', metadata: { model: 'deepseek-v4-flash', response: '[]', finishReason: 'tool_calls', usage: { inputTokens: 1, outputTokens: 1, totalTokens: 2, reasoningTokens: 0, cacheHitTokens: 0, cacheMissTokens: 0, durationMs: null } } }]);
    const { stream } = await streamSubtasksForTask({ task, userId: 'user-1', provider, signal });
